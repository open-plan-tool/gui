...
def forwards(apps, schema_editor):
    ...
    qs = Asset.objects.select_related("asset_type", "input_timeseries").only(
        "id", "asset_type__asset_type", "input_timeseries_id"
    )
    to_update = []
    for asset in qs.iterator():
        ...
        if asset_type not in ALLOWED:
            raise ValueError(
                f"Asset type {asset_type} not in allowed timeseries assets"
            )

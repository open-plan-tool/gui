...
def project_search(request, proj_id=None, scen_id=None):
    ...
    messages.warning(
        ...
        mark_safe(
            _(
                "<p>! change of server !</br></br>Hello dear open-plan-tool user!</br></br> We would like to let you know we moved the tool to a new server on 2025-03-25 around 1am. From now on you should be able to reach the tool on the new server via <a href='https://open-plan-tool.org' >https://open-plan-tool.org</a> with your credentials. Soon <a>https://open-plan.rl-institut.de</a> will be automatically redirected there. </br>Please do no create new projects/scenario here as from now on the new scenarios you are creating here from now on WILL NOT BE transferred to the new server. </br>Instead, create new scenarios on the new server (login with your credentials via provided link). Should you encounter any inconvenience, please let us know at <a>open-plan@rl-institut</a>. Note that we would be able to retrieve your data as we will not erase the data on the old server immediately, however we would like to avoid doing individual project backup. Thank you for your understanding and thank you for using open_plan tool!</br></br>Your open_plan team</p>"
            )
        ),
    )
